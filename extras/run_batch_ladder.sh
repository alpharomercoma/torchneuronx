...
# NEURON CACHES FAILED COMPILES. This is a recorded gotcha in this project and
# it silently invalidated the first trn1 run of this very ladder: micro-batch 2
# failed with NCC_EXTP003, and micro-batch 4 and 8 then reported the IDENTICAL
# instruction count (2064384) because they reused the cached failure instead of
# compiling their own, larger graphs. Three rungs looked like three
# measurements and only one of them was.
#
# The tell was that the instruction count did not change with micro-batch, when
# graph size must grow with it. Any ladder whose rungs can fail needs this flag,
# because the FIRST failure would otherwise be reported for every rung above it.
...
lane() {   # lane <tag> <args...>
  ...
  "$PY" "$TELEM" --out "$OUT/$tag.telemetry.csv" -- \
    ...
    > "$OUT/$tag.log" 2>&1 || {
      ...
      # Record the instruction count when the compiler reports one. It is the
      # audit trail for the cached-failure trap above: counts that do NOT grow
      # with micro-batch mean the rungs were not independently compiled, and the
      # results must be thrown away rather than reported.
      INSTR=$(grep -oE "Instructions generated by compiler [0-9]+" "$OUT/$tag.log" | grep -oE "[0-9]+$" | head -1)
      printf '{"tag":"%s","box":"%s","status":"failed","reason":"%s","compiler_instructions":%s,"captured":"%s"}\n' \
        "$tag" "$BOX" "$REASON" "${INSTR:-null}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$OUT/$tag.failure.json"
      echo "  $tag FAILED (receipt, instructions=${INSTR:-n/a}) -- ladder continues"; return 0; }
