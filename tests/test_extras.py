...
def test_batch_ladder_invalidates_itself_on_shared_module_hashes():
    """The ladder must still be able to call itself SUSPECT -- but on the right
    signal.

    This test used to require the check `len(set(counts)) == 1`, i.e. flag the
    ladder when failed rungs shared an instruction count. That was withdrawn on
    2026-08-12: the count is invariant across a 4x change in the rejected
    operator's tensor volume and across all 59 occurrences in the study, so
    identical counts are the normal result for genuinely independent compiles.
    The check fired on a sound ladder, and the test enforced the false premise.

    Independent compilation means distinct submitted graphs, and the graph
    identity the compiler itself prints is the MODULE_ hash.
    """
    src = _batch_ladder_src()
    assert 'summary["SUSPECT"]' in src, "the ladder must still be able to fail itself"
    assert "len(set(hashes)) == 1" in src, "SUSPECT must key on module hash"
    assert "len(set(counts)) == 1" not in src, (
        "the withdrawn instruction-count heuristic is back; it flags sound ladders"
    )
